...
class BaseSlider {
	...
	setContentWidth() {
		this.sliderContentEL.style.width = `${this.count * 100}%`;
	}

	// 不带动画移动
	move(distance) {
		this.sliderContentEL.style.transform = `translateX(${distance}%)`;
	}
	...
	// 切换上一张
	prev() {
		this.to(currIndex(this.currIndex) - 1);
	}

	// 切换下一张
	next() {
		this.to(parseInt(this.currIndex) + 1);
	}
